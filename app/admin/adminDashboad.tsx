...
export default function AdminDashboard() {
    ...
    const [editingPage, setEditingPage] = useState<Category | null>(null)
    const [newPage, setNewPage] = useState({ title: '', description: '' })
    const [pageToDelete, setPageToDelete] = useState<Category | null>(null)
    const { toast } = useToast()
    const router = useRouter()
    ...
    const handleDeletePage = async (page: Category) => {
        try {
            await axios.delete(`${URL}/file/allProducts/delete/${page._id}`)
            toast({ title: 'Success', description: 'Page deleted successfully', variant: 'default' })
            fetchPages()
        } catch (error) {
                                            ...
                                            <AlertDialog>
                                                ...
                                                <AlertDialogContent>
                                                    <AlertDialogHeader>
                                                        ...
                                                        <AlertDialogDescription>
                                                            This action cannot be undone. This will permanently delete the page and all its contents.
                                                        </AlertDialogDescription>
                                                    </AlertDialogHeader>
                                                    <AlertDialogFooter>
                                                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                        <AlertDialogAction onClick={() => pageToDelete && handleDeletePage(pageToDelete)}>
                                                            Delete
                                                        </AlertDialogAction>
                                                    </AlertDialogFooter>
                                                </AlertDialogContent>
